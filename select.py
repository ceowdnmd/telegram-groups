...
# 创建表
create_table_query = '''
CREATE TABLE IF NOT EXISTS over1k (
...
with tqdm(total=len(data)) as pbar:
    for index, entry in enumerate(data):
        ...
        member_count = float(s_value)
        
        # 仅将用户数量大于 1000 的数据插入到批量列表
        if member_count > 1000:
            formatted_member_count = f"{member_count / 1000:.2f}"
        ...
        if len(batch_insert_list) == batch_size or index == len(data) - 1:
            insert_query = "INSERT INTO over1k (lang, num, title, uid) VALUES (?, ?, ?, ?)"
